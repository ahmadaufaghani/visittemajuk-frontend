...
export async function getPhotoSpotsBanner() {
  const response = await apiRequest<Banner[]>(`/banners?menu=spotfoto`, {
    method: 'GET'
  });
...
export async function createPhotoSpotsBanner(
  payload: FormData,
  token: string
): Promise<Banner> {
...
export async function updatePhotoSpotsBanner(
  id: number,
  payload: FormData,
  token: string
): Promise<Banner> {
